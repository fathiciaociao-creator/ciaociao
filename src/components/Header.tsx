        ...
        <div className="md:hidden">
          {session ? (
            <div className="flex items-center gap-2">
              ...
              <Link href="/my-orders" className="p-2 text-brand-black/60 hover:text-brand-red bg-brand-cream rounded-full border border-gray-200">
                <ShoppingBag size={18} />
              </Link>
            </div>
          ) : (
      ...
      <div className="flex-1 flex items-center justify-end gap-3 md:gap-6">
         ...
         {!isStoreOpen && (
           <div className="flex items-center gap-1.5 md:gap-2 bg-red-50 text-brand-red px-2 py-1 md:px-4 md:py-2 rounded-full border border-red-200 shadow-sm animate-pulse">
             ...
             <span className="text-[8px] md:text-xs font-black tracking-widest uppercase hidden sm:block">Store Closed</span>
             <span className="text-[8px] font-black tracking-widest uppercase sm:hidden">Closed</span>
           </div>
         )}
         ...
         <div className="hidden md:flex">
          {session ? (
            <div className="flex items-center gap-3">
              <Link href="/my-orders" className="flex items-center gap-1 text-brand-black/60 hover:text-brand-red transition-all font-bold text-xs" title="My Orders">
                <ShoppingBag size={18} className="w-4 h-4" /> 
                <span>My Orders</span>
              </Link>
              <button onClick={() => signOut()} className="flex items-center gap-1 text-brand-black/40 hover:text-brand-red transition-all font-bold text-xs ml-2" title="Sign Out">
