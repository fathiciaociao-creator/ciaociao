...
const orderSchema = z.object({
  ...
  orderType: z.enum(['DELIVERY', 'PICKUP', 'TABLE']),
  pickupTime: z.string().nullable().optional(),
  reservationPeople: z.number().int().min(1).nullable().optional(),
  ...
  try {
    ...
    const newOrder = await prisma.order.create({
      data: {
        ...
        reservationPeople: data.reservationPeople || null,
        ...
        discountAmount: parseFloat((calculatedTotal - subtotalAfterDiscount).toFixed(2)),
